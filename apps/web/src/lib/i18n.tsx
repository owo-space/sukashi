...
const JA: Record<string, string> = {
  ...
  "备注": "備考",
  "系统设置": "システム設定",
  "待实现 — 此页面将按 legacy UI 1:1 复刻。": "未実装です。このページは legacy UI に合わせて再現されます。",
  "日本語": "日本語",
  "透かし": "透かし",
  "自由への道": "自由への道",
  "当你的订阅地址或账户发生泄漏被他人滥用时，可以在此重置订阅信息。避免带来不必要的损失。": "サブスクリプションURLまたはアカウントが漏洩して不正利用された場合は、ここでサブスクリプション情報をリセットできます。不要な損失を防げます。",
  "重置订阅": "サブスクリプションをリセット",
  "复制订阅链接": "サブスクリプションリンクをコピー",
  "订阅链接已复制": "サブスクリプションリンクをコピーしました",
  "订阅信息已重置": "サブスクリプション情報をリセットしました",
  "该用户缺少订阅令牌": "このユーザーにはサブスクリプショントークンがありません",
  "复制失败,请手动复制": "コピーに失敗しました。手動でコピーしてください",
  "暂无可选周期": "選択可能な周期はありません",
  "余额 (CNY 分)": "残高 (CNY 分)",
  "流量重置方式 (0 = 月初, 1 = 购买日, 2 = 不重置)": "トラフィックリセット方式 (0 = 月初, 1 = 購入日, 2 = リセットしない)",
  "显示": "表示",
  "生成": "生成",
  "标题": "タイトル",
  "更新时间": "更新日時",
  "添加 Stripe": "Stripe を追加",
  "设备": "端末",
  "设备数": "端末数",
  "— 无 —": "— なし —",
  "流量": "トラフィック",
  "限速": "速度制限",
  "排序": "並び順",
  "关闭后,订阅到期不可续费": "無効にすると、期限切れのサブスクリプションを更新できません",
  "请输入套餐名称": "プラン名を入力してください",
  "请输入套餐描述,支持HTML": "プラン説明を入力してください。HTML に対応しています",
  "套餐流量 (GB)": "プラントラフィック (GB)",
  "设备数限制 (留空不限)": "端末数制限 (空欄で無制限)",
  "留空则不限制": "空欄で無制限",
  "限速 (Mbps,留空不限)": "速度制限 (Mbps、空欄で無制限)",
  "无权限组": "権限グループなし",
  "允许续费": "更新を許可",
  "节点名称及在线状态": "ノード名とオンライン状態",
  "当前在线人数": "現在のオンライン人数",
  "计费倍率,1x 表示按实际流量计费": "課金倍率。1x は実際のトラフィックで課金されます",
  "例如 AU 1": "例: AU 1",
  "1.2.3.4 或 hostname": "1.2.3.4 または hostname",
  "数字越大越靠前": "数値が大きいほど前に表示されます",
  "混淆": "難読化",
  ...
  "直连": "直接接続",
  "动作值 (可选)": "アクション値 (任意)",
  "可选,例如 8.8.8.8": "任意。例: 8.8.8.8",
  "匹配规则 (一行一条)": "マッチルール (1行に1件)",
  "Stripe 支付配置": "Stripe 決済設定",
  "Stripe 结算用的币种": "Stripe 決済に使用する通貨",
  "选择货币": "通貨を選択",
  "选你建的端点": "作成したエンドポイントを選択",
  "Webhook 接收地址": "Webhook 受信URL",
  "把这个地址填到 Stripe Dashboard": "このURLを Stripe Dashboard に入力してください",
  "请先保存,生成 uuid 后再复制": "先に保存して UUID を生成してからコピーしてください",
  "已复制 Webhook 地址": "Webhook URLをコピーしました",
  "点击复制": "クリックしてコピー",
  "编辑优惠券": "クーポンを編集",
  "固定金额 (分)": "固定金額 (分)",
  "金额/Plan": "金額/Plan",
  "金额 (分)": "金額 (分)",
  "使用上限 (留空不限)": "使用上限 (空欄で無制限)",
  "正文 (Markdown / HTML)": "本文 (Markdown / HTML)",
  "图片 URL": "画像URL",
  "正文 (HTML)": "本文 (HTML)",
  "主题配置 — 管理端": "テーマ設定 - 管理画面",
  "此处的设置只影响管理后台。用户端的样式请在「系统配置」→「个性化」中调整。": "ここの設定は管理画面にのみ反映されます。ユーザー画面のスタイルは「システム設定」→「パーソナライズ」で調整してください。",
  "默认蓝": "デフォルトブルー",
  "暗蓝": "ダークブルー",
  "暗黑": "ダーク",
  "青绿": "ティール",
  ...
  "当前网站最新网址,将会在邮件等需要用于网址处使用。": "現在のサイトURLです。メールなどURLが必要な箇所で使用されます。",
  "当站点没有使用HTTPS、CDN或反向代理开启强制HTTPS时需要开启。": "HTTPS、CDN、またはリバースプロキシ側で強制HTTPSが設定されていない場合に有効化してください。",
  "请输入LOGO URL,末尾不要 /": "末尾に / を付けずに LOGO URL を入力してください",
  "用于显示需要LOGO的地方。": "LOGO が必要な場所に表示されます。",
  "订阅URL": "サブスクリプションURL",
  "用于订阅时使用,留空则为站点URL。如果多个订阅URL随机获取使用请使用回车进行分割。": "サブスクリプションで使用します。空欄の場合はサイトURLを使用します。複数のサブスクリプションURLをランダムに使用する場合は改行で区切ってください。",
  "订阅路径": "サブスクリプションパス",
  "用于订阅使用,留空则为/api/v1/client/subscribe。如需要修改不同的订阅路径请设置。": "サブスクリプションで使用します。空欄の場合は /api/v1/client/subscribe です。別のパスを使う場合に設定してください。",
  "用户条款(TOS)URL": "利用規約(TOS)URL",
  "请输入用户条款URL,末尾不要 /": "末尾に / を付けずに利用規約URLを入力してください",
  "用于跳转到用户条款(TOS)。": "利用規約(TOS)へのリンクに使用します。",
  "开启后任何人都将无法进行注册。": "有効にすると誰も登録できなくなります。",
  "注册试用": "登録時のトライアル",
  "选择需要试用的订阅,如果没有选项请先创建订阅管理增加。": "トライアルに使うサブスクリプションを選択してください。選択肢がない場合は先にサブスクリプションを作成してください。",
  "试用时间(小时)": "トライアル時間 (時間)",
  "货币单位": "通貨単位",
  "仅用于展示使用。更改后系统中所有的货币单位都将发生变更。": "表示専用です。変更するとシステム内のすべての通貨単位表示が変わります。",
  "货币符号": "通貨記号",
  "仅用于展示使用。更改后系统中所有的货币符号都将发生变更。": "表示専用です。変更するとシステム内のすべての通貨記号表示が変わります。",
  "注册时强制要求验证邮箱验证码。": "登録時にメール認証コードの確認を必須にします。",
  "允许更换订阅": "サブスクリプション変更を許可",
  "启用剩余价值抵扣": "残存価値の控除を有効化",
  "允许重新选购未到期订阅": "未期限切れサブスクリプションの再購入を許可",
  "显示订阅链接二维码": "サブスクリプションリンクのQRコードを表示",
  "订阅到期提醒天数": "サブスクリプション期限通知日数",
  "流量重置方法 (0/1/2)": "トラフィックリセット方式 (0/1/2)",
  "充值加成 (JSON)": "チャージボーナス (JSON)",
  "工单": "チケット",
  "关闭工单系统": "チケットシステムを無効化",
  "强制邀请注册": "招待登録を必須にする",
  "邀请佣金 (%)": "招待コミッション (%)",
  "邀请码生成上限": "招待コード生成上限",
  "邀请码永久有效": "招待コードを無期限にする",
  "仅首次订单返佣": "初回注文のみコミッション対象",
  "自动审核佣金": "コミッションを自動承認",
  "提现门槛 (CNY)": "出金しきい値 (CNY)",
  "关闭佣金提现": "コミッション出金を無効化",
  "多级佣金分配": "多段階コミッション",
  ...
  "拉取间隔 (秒)": "取得間隔 (秒)",
  "上报间隔 (秒)": "報告間隔 (秒)",
  "节点最小上报流量 (Bytes)": "ノード最小報告トラフィック (Bytes)",
  "在线设备最小流量 (Bytes)": "オンライン端末最小トラフィック (Bytes)",
  "设备限制模式 (0/1)": "端末制限モード (0/1)",
  "SMTP 主机": "SMTP ホスト",
  "SMTP 端口": "SMTP ポート",
  "发件地址": "送信元アドレス",
  "启用 Telegram Bot": "Telegram Bot を有効化",
  "讨论组链接": "ディスカッショングループリンク",
  "Windows 版本号": "Windows バージョン番号",
  "Windows 下载链接": "Windows ダウンロードURL",
  "macOS 版本号": "macOS バージョン番号",
  "macOS 下载链接": "macOS ダウンロードURL",
  "Android 版本号": "Android バージョン番号",
  "Android 下载链接": "Android ダウンロードURL",
  "已复制安装命令": "インストールコマンドをコピーしました",
  "节点已创建。在节点服务器以 root 用户执行下面的脚本即可完成 Sukad 安装与连接。": "ノードを作成しました。ノードサーバーで root ユーザーとして下記スクリプトを実行すると Sukad のインストールと接続が完了します。",
  "在节点服务器以 root 用户执行下面的脚本即可。": "ノードサーバーで root ユーザーとして下記スクリプトを実行してください。",
  "(暂无可用的安装命令)": "(利用可能なインストールコマンドはありません)",
  "(保存后生成)": "(保存後に生成)",
  "批量发送邮件 (TODO)": "メール一括送信 (TODO)",
  "导出 CSV (TODO)": "CSV をエクスポート (TODO)",
  "新密码 (留空保持)": "新しいパスワード (空欄で維持)",
  "到期时间 (留空 = 长期)": "期限 (空欄 = 無期限)",
  "总流量 (GB)": "総トラフィック (GB)",
  "设备限制 (空=不限)": "端末制限 (空欄 = 無制限)",
  "已用上传 (GB)": "使用済みアップロード (GB)",
  "已用下载 (GB)": "使用済みダウンロード (GB)",
  "限速 (Mbps,空=不限)": "速度制限 (Mbps、空欄 = 無制限)",
  "钱包余额 (分)": "ウォレット残高 (分)",
  "佣金余额 (分)": "コミッション残高 (分)"
};
...
function localizeReplacementOutput(source: string, locale: Locale): string {
  if (locale === "zh-TW") return convertToTraditional(source);
